...
template<typename T>
T read(void* buffer, uint64_t byteOffset){
	T value;
...
template<typename T>
T read(void* buffer, int64_t byteOffset) {
	T value;
...
template<typename T>
void write(void* buffer, uint64_t byteOffset, T value){

	uint8_t* buffer_u8 = (uint8_t*)buffer;

	memcpy(buffer_u8 + byteOffset, &value, sizeof(T));
}
...
inline string readTextFile(string path) {
	...
	if (!fs::exists(path)) {
		println("ERROR: tried to read a file that does not exist: '{}'", path);
	}
...
inline shared_ptr<Buffer> readBinaryFile(string path, uint64_t start, uint64_t size = 1'000'000'000'000) {
...
inline bool types_match(B value){
	return dynamic_cast<T>(value) != nullptr;
}

template<class T, class B>
inline bool types_match(B value1, B value2){
	return (dynamic_cast<T>(value1) != nullptr) && (dynamic_cast<T>(value2) != nullptr);
}

template<class WANT, class HAVE>
inline void onTypeMatch(shared_ptr<HAVE> value, std::function<void(shared_ptr<WANT>)> callback){
	bool matches = dynamic_pointer_cast<WANT>(value) != nullptr;

	if(matches){
		callback(dynamic_pointer_cast<WANT>(value));
	}
}
