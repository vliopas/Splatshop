    ...
    uint32_t radixShift)
{
	...
	// 	printf("=================================================\n");
	// 	printf("sort:             %llu \n", sort);
	// 	printf("sortPayload:      %llu \n", sortPayload);
	// 	printf("alt:              %llu \n", alt);
	// 	printf("altPayload:       %llu \n", altPayload);
	// 	printf("passHistogram:    %llu \n", passHistogram);
	// 	printf("index:            %llu \n", index);
	// 	printf("size:             %d \n", size);
	// 	printf("radixShift:       %d \n", radixShift);
	// }
    ...
    if (partitionIndex < gridDim.x - 1)
    {
            ...
            ++i, t += blockDim.x)
        {
			...
			// printf("alt[%d] = %d \n", s_localHistogram[digits[i]] + t, s_warpHistograms[t]);
        }
        __syncthreads();
