    ...
    vector<ConcurrentTarget> targets)
{
    ...
    for (auto target : targets)
    {
        ...
        void *argsCreatefragmentArray[] = {
            ...
            &target.cptr_numFragments,
            // output
            &target.virt_tileIDs->cptr,
            &target.virt_indices->cptr,
        };
    ...
    for (ConcurrentTarget concurrent : concurrentTargets)
    {
        ...
        { // RENDER TRIANGLES
            ...
            if (triangleQueue.size() > 0)
            {
                ...
                static CUdeviceptr cptr_queue_bins = 0;
                static CUdeviceptr cptr_queue_bin_counter = 0;
                static CUdeviceptr cptr_queue_binRasterized_counter = 0;
                ...
                if (cptr_queue_geometry == 0)
                {
                    ...
                    cptr_queue_bins = CURuntime::alloc("queue_material", 500'000'000);
                    cptr_queue_bin_counter = CURuntime::alloc("queue_material", 4);
                    cptr_queue_binRasterized_counter = CURuntime::alloc("queue_material", 4);
                }
                ...
                switch (SplatEditor::instance->settings.rasterMode) {
                case RASTERIZE_SMALL_ONLY:
                    prog_triangles->launchCooperative(
                        "kernel_drawTriangleQueue", { &launchArgs, &queue, &target }, settings
                    );
                    break;

                case RASTERIZE_PERSISTENT_KERNEL:
                    prog_triangles->launchCooperative(
                        "kernel_persistentDraw", { &launchArgs, &queue, &target }, settings
                    );
                    break;

                case RASTERIZE_2_PHASE:
                    cuMemsetD32Async(cptr_queue_bin_counter, 0, 1, mainstream);
                    cuMemsetD32Async(cptr_queue_binRasterized_counter, 0, 1, mainstream);

                    prog_triangles->launchCooperative("kernel_storeBins", 
                        { &launchArgs, &queue, &target, &cptr_queue_bins, &cptr_queue_bin_counter}, settings);
                    prog_triangles->launchCooperative("kernel_rasterizeBins", { 
                        &launchArgs, 
                        &queue, 
                        &target, 
                        &cptr_queue_bins, 
                        &cptr_queue_bin_counter, 
                        &cptr_queue_binRasterized_counter}
                        , settings);
                    break;
                }
            }
